...
func (s *PostgresStorage) CreateOrder(customerID int, orderDate, dueDate string) (int, error) {
	var id int
	err := s.DB.QueryRow("INSERT INTO orders (customer_id, order_date, shipment_due) VALUES ($1, $2, $3) RETURNING id", customerID, orderDate, dueDate).Scan(&id)
	return id, err
}

func (s *PostgresStorage) AddItemToOrder(orderID int, name string, size, color *string, price float64) error {
	_, err := s.DB.Exec("INSERT INTO items (order_id, name, size, color, price) VALUES ($1, $2, $3, $4, $5)", orderID, name, size, color, price)
	return err
}
